{{--
    resources/views/owner/listings/_form.blade.php
    Shared fields for both create and edit listing modals.
    Expects: $listing (null when creating), $types (array)

    IMPORTANT: this partial is included once per modal (1 create + N edit
    modals on the same page), so it must NOT rely on global element IDs
    like #type - duplicate IDs would only let the first instance work.
    Instead every field is suffixed with a unique id and the toggle script
    is scoped via closest('form').
--}}
@php
    $uid = $listing ? 'edit-'.$listing->id : 'new';
@endphp
...
<script>
    // Scoped to the <select> that fired the change event - safe with N copies of this form on one page.
